...
def detect_web(path):
    ...
    # 	print(notes.web_entities.entity.score)

    	# print(entity)

        # for i in range(len(notes.web_entities)):
        	# print(len(notes.web_entities))
      #   	possible_entity.append(entity[i].score)
    		# entity_description.append(entity[i].description)

    	# finds the item with the highest score and the index in the list
    	# location = possible_entity.index(max(possible_entity))
    	
    	# displays result of what item was scanned
    	# print("A(n) " + entity_description[location] + ' was placed into the system')
    # # puts the score and descroption of the items into their own list 
    # # so that we can do operations
    # for i in range(len(notes.web_entity)):
    # 	possible_entity.append(entity[i].score)
    # 	entity_description.append(entity[i].description)
